...
class EGFNet(nn.Module):
    ...
    def forward(self, rgb, depth, edge):
        ...
        ir1 = self.layer1_ir(ir)

        x2 = self.layer2_rgb(x1)
        ir2 = self.layer2_ir(ir1)


        x3 = self.layer3_rgb(x2)
        ir3 = self.layer3_ir(ir2)

        x4 = self.layer4_rgb(x3)
        ir4 = self.layer4_ir(ir3)


        x5 = self.layer5_rgb(x4)
        ir5 = self.layer5_ir(ir4)
        ...
        s5, out5 = self.MFM5(x5, ir5)
        s4, out4 = self.MFM4(x4, ir4)
        b3, out3 = self.MFM3(x3, ir3)
        b2, out2 = self.MFM2(x2, ir2)
        b1, out1 = self.MFM1(x1, ir1)



        S = self.sgm(s5, s4, edge)
