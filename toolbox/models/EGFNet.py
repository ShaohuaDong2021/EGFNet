...
class DEM(nn.Module):
    def __init__(self, channel, reduction=16):
        ...
        # senet use global average pooling  but this use global max pooling
...
class vgg145withoutm(nn.Module):
    def __init__(self, n_classes):
        super(vgg145withoutm, self).__init__()
    ...
    def forward(self, rgb, depth, label):
        ...
        high2 = torch.nn.functional.interpolate(f4, scale_factor=2, mode='bilinear')
        out21 = self.resf2(f3, out2, high2)
        f2 = torch.nn.functional.interpolate(out21, scale_factor=2, mode='bilinear')


        high1 = torch.nn.functional.interpolate(f4, scale_factor=4, mode='bilinear')
        out11 = self.resf1(f2, out1, high1)  # 64  64   ->  64 or n_class
